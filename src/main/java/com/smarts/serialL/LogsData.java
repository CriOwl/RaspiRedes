...
public class LogsData {
    ...
    private final String[] parametersBytes = {
        ...
        public LogsData() {
            ...
            secondLogsData();
            thirdLogsData();
            getFaultLogs("/home/EPI5/Smarts/faultLogs"+id+".csv");
            getAuditLogs("/home/EPI5/Smarts/AuditLogs"+id+".csv");
            getAlarmanLogs("/home/EPI5/Smarts/AlarmanLogs"+id+".csv"); 
        ...
        private void getDataLogs(byte[] starAddressdataLogs, int countLogs, int numberLog,String logCollection,String path) {
            if (countLogs > 2000) {
                return;
            }
            if (countLogs > 504) {
                ...
                for (int i = logsWriter; i < 504; i++) {
                    ...
                    traslateAllData(Arrays.copyOfRange(lastloggin, 8, (loggerList.size() * 4) + 8));
                    Dao.addJsonCollection(toJsonData(), ConfigM2.getLog1Collection(), id.toString(),"Log"+numberLog);
                    starAddressdataLogs = calculateNextAdreesLogs(starAddressdataLogs, loggerList.size() * 4);
                }
                for (int i = 1; i < logsWriter; i++) {
                    ...
                    traslateAllData(Arrays.copyOfRange(lastloggin2, 8, (loggerList.size() * 4) + 8));
                    Dao.addJsonCollection(toJsonData(), ConfigM2.getLog1Collection(), id.toString(),"Log"+numberLog);
                    startAddressOld = calculateNextAdreesLogs(startAddressOld, loggerList.size() * 4);
                }
                ManageComunications.sendDocuments(numberLog, Dao.generateCSV(logCollection, path));
                return;
            }
            for (int i = 1; i < countLogs; i++) {
                ...
                traslateAllData(Arrays.copyOfRange(lastloggin3, 8, (loggerList.size() * 4) + 8));
                Dao.addJsonCollection(toJsonData(), ConfigM2.getLog1Collection(), id.toString(),"Log"+numberLog);
                starAddressdataLogs = calculateNextAdreesLogs(starAddressdataLogs, loggerList.size() * 4);
            }
            ManageComunications.sendDocuments(numberLog, Dao.generateCSV(logCollection, path));            
        ...
        public void firstLogsData() {
            getDataLogs(startAddressLogs1, ptrLog1,1, ConfigM2.getLog1Collection(),"/home/EPI5/.Smarts/logs1"+id+".csv");
        }
        
        public void secondLogsData() {
            getDataLogs(startAddressLogs2, ptrLog2,2, ConfigM2.getlog2Collection(),"/home/EPI5/.Smarts/logs2"+id+".csv");
            
            
        }
        
        public void thirdLogsData() {
            getDataLogs(startAddressLogs3, ptrLog3,3, ConfigM2.getlog3Collection(),"/home/EPI5/.Smarts/logs3"+id+".csv");
        }
        
        private void getLastLogs(byte[] starAddressdataLogs, int countLogs, int numberLog,String logCollection) {
            if (countLogs > 2000) {
                return;
            }
            Dao.dropCollection(logCollection);
            if (countLogs > 504) {
                int logsWriter = countLogs - 504;
                ...
                traslateAllData(Arrays.copyOfRange(lastloggin, 8, (loggerList.size() * 4) + 8));
                Dao.addJsonCollection(toJsonData(), logCollection, id.toString(),"Log"+numberLog);
                ManageComunications.sendJsonLogs(numberLog, toJsonData());
                return;
            }
            for (int i = 1; i < countLogs; i++) {
                starAddressdataLogs = calculateNextAdreesLogs(starAddressdataLogs, loggerList.size() * 4);
            ...
            traslateAllData(Arrays.copyOfRange(lastloggin, 8, (loggerList.size() * 4) + 8));
            Dao.addJsonCollection(toJsonData(), logCollection, path,"Log"+numberLog);
            ManageComunications.sendJsonLogs(numberLog, toJsonData());
        }

        public void getLastlog1() {
            System.out.println("Lastlogs");
            getLastLogs(startAddressLogs1, ptrLog1,1,ConfigM2.getLog1Collection());
        ...
        public void getLastlog2() {
            ...
            getLastLogs(startAddressLogs2, ptrLog2,2,ConfigM2.getlog2Collection());
            
        }
        
        public void getLastlog3() {
            System.out.println("Lastlogs3");
            getLastLogs(startAddressLogs3, ptrLog3, 3,ConfigM2.getlog3Collection());
        }
        
        private void getAlarmanLogs(String nameLogs) {
            try {
                Dao.dropCollection(ConfigM2.getAlarmCollection());
                for (int i = 1; i < getPtrLogAlarm(); i++) {
                    ...
                    Dao.addJsonCollection(toJsonAlarm(), ConfigM2.getAlarmCollection(), id.toString(),"AlarmLogs");
                    startAddressAlarm = calculateNextAdreesLogs(startAddressAlarm, 5);
                }
                ManageComunications.sendDocuments(4, Dao.generateCSV(ConfigM2.getAlarmCollection(), "/home/EPI5/.Smarts/Alarm_"+id+".csv"));
            } catch (Exception e) {
        ...
        public void getLastAlarmLogs() {
            try {
                ...
                JsonAlarm = dataWriter;
                Dao.addJsonCollection(toJsonAlarm(), ConfigM2.getAlarmCollection(), id.toString(),"AlarmLogs");
                ManageComunications.sendJsonLogs(4, toJsonAlarm());
            } catch (Exception e) {
                System.out.println(e);
            }
        }
        ...
        public void getLastFaultLogs() {
            try {
                ...
                Dao.addJsonCollection(toJsonFault(), ConfigM2.getFaultCollection(), id.toString(),"FaultLogs");
                ManageComunications.sendJsonLogs(5, toJsonFault());
            } catch (Exception e) {
                System.out.println(e);
            }
        }
        
        private void getFaultLogs(String nameLogs) {
            try {
                Dao.dropCollection(ConfigM2.getFaultCollection());
                for (int i = 1; i < getPtrLogFault(); i++) {
                    byte[] dataFault = SerialHelper.stablishConnection(
                    SerialHelper.createDataRequestPacketAddress((byte) 0x72, (byte) startAddressFaultLog[0],
                    startAddressFaultLog[1], (byte) 0x05),
                ...
                ManageComunications.sendDocuments(5, Dao.generateCSV(ConfigM2.getFaultCollection(), "/home/EPI5/.Smarts/Fault_"+id+".csv"));                
            } catch (Exception e) {
            }
        ...
        private void getAuditLogs(String nameLogs) {
            startAddressAudit = startAddressLogginArea;
            Dao.dropCollection(ConfigM2.getAuditCollection());
            ...
            if (maxAuditLogs > 0) {
                try {
                    ...
                    dataWriter = getId() + dataAndTime(
                    ByteBuffer.wrap(Arrays.copyOfRange(firstAudit, 8, 12)).order(ByteOrder.LITTLE_ENDIAN).getInt())
                    + ";"
                    + traslateDataAuditlog(firstAudit) + ";";
                    JsonAudit = dataWriter;
                    Dao.addJsonCollection(toJsonAudit(), ConfigM2.getAuditCollection(), id.toString(),"AuditLogs");
                    ...
                    ManageComunications.sendDocuments(6, Dao.generateCSV(ConfigM2.getAuditCollection(), "/home/EPI5/.Smarts/Audit_"+id+".csv"));                
        ...
        private String toJsonAudit() {
            try {
                Scanner separador = new Scanner(JsonAudit).useDelimiter(";");
                String Json = "{" + '"' + "Id" + '"' + ": " + getSerialRaspi() + ","
                + '"' + "SERIAL" + '"' + ": " + getId() + ","
                + '"' + "Date_and_Time" + '"' + ": " + separador.next() + ","
                + '"' + "Parameter" + '"' + ": " + separador.next() + "}";
